...
- Windows Forms(문법 학습) : 가장 오래된 윈앱 개발
...
- 굵은 글씨체로 나오는 프로젝트가 시작 프로젝트
    1. 시작 프로젝트로 변경 할 프로젝트 > 마우스 오른쪽 > 시작 프로젝트로 설정
    2. 전체 솔루선 > 속성 > 속성 페이지
...
### 윈폼 앱 / 윈앱 개발 순서
1. Forms1.cs 이름 변경 : FrmMain.cs
    - 변싱 시 `모즌 참조 이름도 바꾸시겠습니까?` 메시지 창
        - 반드시 `예` 선택

2. FrmMain.cs [디자인] 클릭 오픈
    - 마우스로 크기 조정

3. 속성 창(F4) 오픈, 솔루션 탬식기 아래 쪽 드래그 후 붙이기
...
11. 오류 발생 시
    - 폼 디자인의 컨트롤 이벤트와 로직 코드 상의 이벤트 처리 메서드 생성 상의 불일치로 발생
    - 이벤트 이름은 생성되고, 이벤트 정의 메서드는 생성이 안되서 발생

    <img src="./image/cs0002.png" width="600">

    - FormMain.Designer.cs 파일 오픈 후 오류난 이벤트 이름 삭제
    - Windows Form Designer generated code 확장
    - 오류난 이벤트 이름 삭제
        - BtnMsg.Click += ....
        - BtnOK.Click += ....
...
14. VS 에서 더블 클릭 함부로 하지 말 것!!!
    - 이벤트 메서드 생성, 컨트롤 자동 생성, 폼 디자인 생성
...
- 기본 문법 - C++, JAVA와 거의 동일. C++와의 차이점만 비교
    1. 기본구조 및 주석 - [소스](./day02/Day02Study/ConsoleApp2/Program.cs)
        ...
        {   
            ...
            {
                /// <summary>
                /// 주석 - XML WNTJR. 소스코드 자동 문서 생성 시 사용.
                /// </summary>
    ...
    2. 자료형과 변수, Nullable - [소스](./day02/Day02Study/SyntaxWinApp01/FrmMain.cs)
        - 변수 사용은 C++과 유사, 닷넷타입이 더 추가 됨
        - 자료형도 C++ 유사. 닷넷타입은 차이가 있음
        - 닷넷타입은 여러 언어에서 공통으로 사용하기 위해서 추가된 기능
        - Nullable : 특수 타입.  데이터 타입 뒤에 ? 추가. null 할당 가능
...
- 각 IDE별 한글 깨짐 해결 방법
    - VS Code > 오른쪽 하단 상태 표시 줄 UTF-8 클릭 인코딩 하여 다시 열기 또는 인코딩하여 저장 실행
...
- `MaskedTextBox` : 사용자에게 주어진 형태로만 입력을 하도록 강제하는 테스트 입력 컨트롤
    - 텍스트박스와 동일하게 Txt~ 로 시작
    - Button 컨트롤과 속성 동일
    - Mask : 사용자에게 입력 제약할 포맷

- GroupBox : 관련있는 컨트롤들을 묶어주는 컨트롤
    - 이름을 절대 바꾸지 않음 (0.1% 정도 변경)
    - Button 컨트롤과 속성 동일
    - Text 속성만 사용
...
- Button 이미지 추가
    1. 각 폼별 리소스 사용
        ...
        - 리소스 선택 > 로컬 리소스 > 가져오기
        - FrmMain.resx에 저장
    2. 폼 위에 ImageList 컨트롤 사용 (ImageIndex에 이미지 설정)
        - 도구 상자 `ImageList` 컨트롤 드래그
        - 폼에 표현되는 컨트롤 아님
        - 이미지 사이즈를 개발자 조정가능
    3. 전체 리소스에 추가
        - Properties > Resoures.resx
        - 속성 Image : ... 버튼
...
- `PictureBox` : 이미지를 표현해주는 컨트롤
    ...
    - BackColor : 폼 색과 다르게 해서 영역표시
    - Enabled : 사용 여부
    - Image : 표시할 이미지 선택
    - Size : 크기
    - SizeMode : 들어가는 이미지 사이즈 형태
        - Normal : 큰 이미지는 잘려서 나옴
        - StretchImage : PictureBox 크기에 맞춰서 표시
...
### VS 개발 팁
- 코드조각(Code Snippet)을 최대한 활용(코딩 양 줄임, 오타 예방)
- 보기 > 개체 브라우저에서 필요한 클래스 검색
- 소스코드가 회색으로 연하게 변하는 것 > 되도록이면 사용 피하라는 뜻 > Alt + Enter로 변경 권장 사항 확인

### C# 기본문법
- 기본문법
    1. 형변환 - [소스](./day04/Day04Study/SyntaxWinApp01/FrmMain.cs)
        - 큰 바이트 데이터형에 작은 바이트형 값을 할당하면 문제 없이 사용 가능(묵시적 형변환)
    ...
    2. 예외처리
        ...
        https://github.com/user-attachments/assets/282ba4d7-8e87-48d7-b973-3c239d1cca2d
...
### 윈앱 컨트롤 4
- `StatusStrip` : 상태 표시줄 컨트롤. 서브 컨트롤 추가 가능
...
- 기본문법
    1. 델리게이트(대리자), 이벤트 - [소스](./day05/Day05Study/SyntaxWinApp01/FrmMain.cs)
        ...
        - Form 오픈 이벤트 및 생성자
            ...
            4. FrmMain_Shown 이벤트 : 폼이 바탕화면 그려질때 이벤트
                - 잘 사용 안함
        ...
        - 실제 실무에서는
            - FrmMain() 생성자, FrmMain_Laod(), FrmMain_FormClosing() 으로 대부분 작업
...
- 고급 문법
    ...
    2. LINQ - C# 3.0부터 도입
        - Language Integrated Query : 언어에서 통합된 쿼리
        - Database 학습 SQL과 유사
        - 데이터를 SQL처러 Query 할 수 있는 문법
    ...
    3. 속성 - Property
        ...
        - UI 클래스(속성) : UI 상태나 외형, 기능 컨트롤
            - 폼 디자인, 코드 둘 다 사용
        - Form.Desinger.cs 내용은 폼 디자인에서 마우스나 디자인 설정에 변경하는 값으로 되도록이면 수정하지 말 것
        - Form_Load 이벤트에 코딩으로 초기화

    4. partial 클래스 : C#에만 존재
        - 나누어진 같은 이름의 클래스를 컴파이 시 하나로 합쳐주는 기능
...
### 윈앱 컨트롤 5
- `BackgroundWorker` : 화면 뒷단에서 잡업할 일 중 스레드 처리가 되어야할 부분을 손쉽게 동작시켜주는 컨트롤(화면표시X)
    - Bgw~ 이름을 사용
    - 일반 속성은 잘 사용안함. 이벤트 3가지는 모두 사용
    - `DoWork` : 백그라운드(스레드) 작업
    - `ProgressChanged` : UI상 변화를 처리 작업
    - `RunWorkerCompleted` : 작업 완료시 처리
...
- `TreeView` : 탐색기 하드디스크, 폴더 나타내는 부분과 동일한 컨트롤
    ...
    - AfterSelect, AfterExpand, AfterCollapse 이벤트롤 주로 사용

- `ListView` : 탐색기 오른쪽 목록을 표시, 사용하는 컨트롤
    - Lsv~
...
- `MonthCalendar` : 스케줄 등록을 위한 컨트롤
    - Cal~ 로  이름 사용
    - FirstDayWeek : 한주 시작일을 설정. default는 일요일 부터
...
- `MenuStrip` : 프로그램 메뉴 관리
    ...
    - 메뉴 이름 작성 시 (&X)와 같이  사용하면 영문자 아래 밑줄 표시
    - Click : 해당 메뉴 클릭 이벤트
...
### C# 문법
- C# 기본
    - enum : 열거형, 필요한 목록의 수로 된 키 값을 문자로 변경해서 사용하는 구조체
    - 이진 &(AND), |(OR) 연산으로 처리할 때 효과적
        ...
        ``` 
    - 모달창, 모달리스창 : 창 위에 다른 창을 띄울때 접근 권한 제어 형태
        - Modal : 부모창과 연관된 작업을 할 때 사용. 모달창이 닫히지 않으면 부모창을 제어할 수 없음
        - Modaless : 부모창과 상관없이 동작. 메인창을 언제나 닫을 수 있고, 메인창은 받으면 모두 종류. 사용 시 주의
...
### C# 응용 - WPF
- WPF 개요
    - `Windows Persentation Foundation` 약자. 윈도우에서 표시하기 위한 기초
    - 윈앱(윈폼즈)을 개선한 버전. 이미지 표현이 비트맵방식(2D). 용량문제, 계단현상 문제가 존재
    - 비트맵 방식의 문제를 해결 -> 벡터방식(텍스트). 속도개선, 용량 절약
...
- WPF 특징
    ...
    - 백엔드는 C# 윈앱가 거의 동일, 프론트엔드만 차이
        - 윈앱과는 다르게 코딩으로 디자인을 수행. 윈앱 처럼 디자이너 사용 가능
...
- XAML
    ...
    - 태그형태로 디자인. <열린태그></닫힌태크>, <태크 내 다른 태그 없을때 />
...
- 데이터바인딩 - [디자인](./day07/Day07Study/WpfStudyApp04/MainWindow.xaml) / [소스](./day07/Day07Study/WpfStudyApp04/MainWindow.xaml.cs)
    ...
    - DataContext : Xaml이 데이터ㅏ를 어디에서 가져오는지 설정하는 속성
        - 데이터 한건만 설정X
        - 클래스 객체를 패키지로 전달 가능
    - 바인딩 순서
...
- 데이터베이스 데이터 바인딩
    ...
    3. 비하인드 코드에서 MySQL 소스 추가
        ...
        6. 트랜잭션객체 : 옵션. 트랜잭션(INSERT, UPDATE, DELETE) 처리 시 필요
...
- 네이게이션 - [소스](./day08/Day08Study/WpfStudyApp02/MainWindow.xaml)
    - 화면 전환
    - 메뉴 클릭으로 화면전환
    - Window, Page 컨트롤
        - Window : Main
